...
public class NioClientHandle implements Runnable{
    ...
    private void handleInput(SelectionKey key) throws IOException {
        if(key.isValid()){
            /*获得关心当前事件的channel   相当于反推，socketChannel前面注册了连接事件*/
            ...
            * 但是三次握手未必就成功了，所以需要等待握手完成和判断握手是否成功*/
            ...
            if(key.isReadable()){
                ...
                /*链路已经关闭，释放资源（四次挥手）*/
                    ...
                    /*取消键，取消的键不会立即从 selector 中移除,而是添加 到 cancelledKeys 中,
                    在下一次 select 操作时移除它.所以在调用某个 key 时,需要使 用 isValid 进行校验.*/
                    key.cancel();
                    sc.close();
                }
    ...
    /*进行连接，连接的时候是进行TCP连接需要进行三次握手才能建立连接*/
    private void doConnect() throws IOException {
        ...
        if(socketChannel.connect(new InetSocketAddress(host,port))){//一旦调用了connect就会有‘连接就绪事件’
            /*连接成功，关注读事件*/
            socketChannel.register(selector,SelectionKey.OP_READ);
        }
        else{
